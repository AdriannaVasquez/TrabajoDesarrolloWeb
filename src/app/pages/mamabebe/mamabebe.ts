...
@Component({
  ...
  templateUrl: './mamabebe.html'
})
export class Mamabebe implements OnInit {
  productosCategoria4: Producto[] = [];
  ...
  cargarProductos() {
    ...
    this.http.get<any>(this.apiUrl).subscribe({
      next: (data) => {
        this.cargando = false;
        if (data.isSuccess) {
          this.productosCategoria4 = data.data.filter((p: Producto) => p.id_categoria === 4);
          this.productosFiltrados = [...this.productosCategoria4];
          this.cargarMarcas();
        } else {
  ...
  cargarMarcas() {
    ...
    this.productosCategoria4.forEach((producto) => {
  ...
  aplicarFiltros() {
    let productosFiltrados = [...this.productosCategoria4];
