...
export class Vitaminas implements OnInit {
  productosCategoria5: Producto[] = [];
  ...
  cargarProductos() {
    ...
    this.http.get<any>(this.apiUrl).subscribe({
      next: (data) => {
        this.cargando = false;
        if (data.isSuccess) {
          this.productosCategoria5 = data.data.filter((p: Producto) => p.id_categoria === 5);
          this.productosFiltrados = [...this.productosCategoria5];
          this.cargarMarcas();
        } else {
  ...
  cargarMarcas() {
    ...
    this.productosCategoria5.forEach((producto) => {
  ...
  aplicarFiltros() {
    let productosFiltrados = [...this.productosCategoria5];
